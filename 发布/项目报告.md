...
`Paper 26.x` · `Java 25` · `MC 26.2 + Fabric 0.19.5` · `v1.1.0` · `MIT`
...
| 文件 | 类型 | 放到哪里 | 谁需要 |
|---|---|---|---|
| `【服务器插件】BlazeHwidBan-1.1.0-mc26.jar` | 服务端插件 | 服务端 `plugins\` | Paper 26.x 服务器（本机测试服就是这条线） |
...
| 13 | 多服务器封禁同步 | ✅ | `sync.backend`: json(默认)/sqlite/mysql，多服共用封禁库，任一服封禁全网 5 秒内生效并踢出在线命中者 |
| 14 | 指纹突变告警 | ✅ | 进服指纹与历史都不同时提醒管理员（改设置/装模组也会变，仅供参考，默认开） |
| 15 | 同机多账号自动处罚 | ✅ | 同一机器码账号数达阈值触发：alert(默认) / tempban 整机临时封禁（默认关） |
...
| `alt-auto-action` | 同机多账号自动处罚 | `enabled`(默认 false) / `max-accounts`(5) / `action`(alert\|tempban) / `tempban-duration`(7d) / `reason` |
| `fingerprint-alert` | 指纹突变告警 | `enabled`（默认 true，仅提醒不处罚） |
| `sync` | 多服务器封禁同步 | `backend`(json\|sqlite\|mysql) / `interval-seconds`(5) / `sqlite.file` / `mysql.*` 连接信息 |
| `audit-log` | 审计日志开关 | — |
...
|---|---|
| `config.yml` | 配置 |
| `bans.json` | 机器码封禁库（`sync.backend: json` 时使用；切到 sqlite/mysql 后仅作迁移源保留） |
| `bans.db` | SQLite 封禁库（仅 `sync.backend: sqlite` 时生成） |
| `profiles.json` | 玩家档案：玩家名 ↔ 机器码 ↔ 指纹、绑定时间、最后活跃（仅本服有效，不参与多服同步） |
...
│  └─ src\main\
│     ├─ java\cn\blaze\hwidban\      17 个类 / 约 2400 行
│     │  ├─ HwidBanPlugin.java       主类 / 模块接线
│     │  ├─ command\                 命令实现
│     │  ├─ hwid\                    封禁库与档案模型
│     │  ├─ storage\                 封禁存储抽象 (json / sqlite / mysql)
│     │  ├─ sync\                    多服封禁同步 (轮询对账 + 即时踢人)
│     │  ├─ fingerprint\             客户端指纹
...
| 日期 | 版本/更新 | 类型 | 摘要 |
|---|---|---|---|
| 2026-09-12 | v1.1.0 | 新功能 | 多服封禁同步(sqlite/mysql)、指纹突变告警、同机多账号自动处罚；mod 无变化 |
| 2026-09-12 | 下线 1.21.x + 构建归位 | 维护 | 只保留 Paper 26.x 线；发布 GitHub；%TEMP% 构建目录改仓库直构 |
...
7. **多服同步的边界** —— sqlite 仅建议同机多服（跨机器共享 SQLite 文件不可靠）；跨机器用 mysql。同步的是封禁库，指纹档案不同步（各服盐值不同）。`alt-auto-action` 与同机检测基于本服档案数据。插件首次启动需联网下载 SQL 驱动。
8. **指纹突变告警会误报** —— 玩家改设置/装模组/换渲染距离都会变指纹，该功能只做参考提醒。
...
**BlazeHwidBan v1.1.0** · 更新于 2026-09-12
